...
class TestBureaucratKnowledgeBase(
    ReduceLoggingMixin, AccessRulesFixMixinST, SavepointCase
):
    @classmethod
    def setUpClass(cls):
        ...
        cls.document_subcat_2 = cls.env.ref("bureaucrat_knowledge.document_demo_sub_2")
    ...
    @hide_log_messages(
        ...
        lambda r: not any(
            [
                r.msg.strip().startswith("The requested operation cannot be completed"),
            ]
        ),
    )
    @hide_log_messages(
        ...
        lambda r: not any(
            [
                r.msg.strip().startswith("Access Denied by record rules for operation"),
            ]
        ),
    )
    @hide_log_messages(
        "odoo.addons.base.models.ir_model",
        lambda r: not r.msg.strip().startswith("Access Denied by ACLs for operation"),
    )
