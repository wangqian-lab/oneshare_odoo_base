...
class BureaucratKnowledgeDocument(models.Model):
    ...
    document_preview_text = fields.Text(compute="_compute_preview", store=True)
    document_preview_image = fields.Binary(
        "Preview", attachment=True, compute="_compute_preview", store=True
    )
    ...
    index_document_body = fields.Text(store=True, compute="_compute_index_body")
    ...
    created_by_id = fields.Many2one(
        ...
        ondelete="restrict",
        index=True,
        help="Document was created by this user",
        copy=False,
    )
    ...
    _sql_constraints = [
        (
            ...
            "Document must have a parent category " "to set Visibility Type 'Parent'",
        ),
    ]
    ...
    def _compute_preview(self):
        for rec in self:
            if rec.document_type == "pdf":
                rec.document_preview_image = rec._get_preview_from_pdf()
            if rec.document_type == "html":
                rec.document_preview_text = _get_preview_from_html(
    ...
    def action_view_history(self):
        ...
        return self.env["generic.mixin.get.action"].get_action_by_xmlid(
            "bureaucrat_knowledge" ".action_bureaucrat_knowledge_document_history",
    ...
    @post_write("name", "document_type", "document_body_html", "document_body_pdf")
