...
class BureaucratKnowledgeDocumentHistory(models.Model):
    ...
    document_type = fields.Selection(selection=DOC_TYPE, required=True)
    ...
    date_create = fields.Datetime(
        default=fields.Datetime.now, index=True, required=True, readonly=True
    )
    ...
    def name_get(self):
        result = []
        for rec in self:
            result.append((rec.id, "%s [%s]" % (rec.document_id.name, rec.date_create)))
        return result
