...
class BureaucratKnowledgeCategory(models.Model):
    ...
    child_ids = fields.One2many("bureaucrat.knowledge.category", "parent_id")
    child_category_count = fields.Integer(compute="_compute_child_category_count")
    document_ids = fields.One2many("bureaucrat.knowledge.document", "category_id")
    documents_count = fields.Integer(compute="_compute_documents_count")
    active = fields.Boolean(default=True, index=True)

    category_contents = fields.Html(compute="_compute_category_contents")
    ...
    _sql_constraints = [
        (
            ...
            "Category must have a parent category" " to set Visibility Type 'Parent'",
        ),
    ]
    ...
    def init(self):
        ...
        tools.drop_view_if_exists(
            self.env.cr, "bureaucrat_knowledge_category_actual_owner_groups_rev_view"
        )
        self.env.cr.execute(
            sql.SQL(
                """
        ...
        tools.drop_view_if_exists(
            self.env.cr, "bureaucrat_knowledge_category_actual_owner_users_rel_view"
        )
        self.env.cr.execute(
            sql.SQL(
                """
        ...
        tools.drop_view_if_exists(
            self.env.cr, "bureaucrat_knowledge_category_actual_editor_groups_rel_view"
        )
        self.env.cr.execute(
            sql.SQL(
                """
        ...
        tools.drop_view_if_exists(
            self.env.cr, "bureaucrat_knowledge_category_actual_editor_users_rel_view"
        )
        self.env.cr.execute(
            sql.SQL(
                """
    ...
    def _post_active_changed(self, changes):
        for rec in self:
            self.with_context(active_test=False).search(
                [("parent_id", "child_of", rec.id), ("active", "!=", rec.active)]
            ).write({"active": rec.active})
            self.env["bureaucrat.knowledge.document"].with_context(
                active_test=False
            ).search(
                [("category_id", "child_of", rec.id), ("active", "!=", rec.active)]
            ).write(
                {"active": rec.active}
            )
