...
class GenericTag(models.Model):
    ...
    category_id = fields.Many2one(
        "generic.tag.category", "Category", index=True, ondelete="restrict"
    )
    name = fields.Char(required=True, translate=True, index=True)
    code = fields.Char(
        index=True,
        help="May be used for special " "tags which have programmed bechavior",
    )
    ...
    category_name = fields.Char(
        related="category_id.name", store=True, index=True, string="Name of Category"
    )
    objects_count = fields.Integer(
        string="Objects",
        compute="_compute_objects_count",
        store=False,
        readonly=True,
        tracking=True,
    ...
    def _compute_objects_count(self):
        for tag in self:
            ...
            try:
                ...
                tag.objects_count = TagModel.search_count([("tag_ids.id", "=", tag.id)])

    @api.depends("category_id.name", "name")
    def _compute_complete_name(self):
        for tag in self:
            if tag.category_id:
                tag.complete_name = "%s / %s" % (tag.category_id.name, tag.name)
            else:
        ...
        for tag in self:
            if tag.category_id and tag.model_id != tag.category_id.model_id:
                raise exceptions.ValidationError(
                    _("Category must be bound to same model as tag")
                )
