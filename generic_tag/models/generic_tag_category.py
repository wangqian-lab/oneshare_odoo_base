...
class GenericTagCategory(models.Model):
    ...
    tag_ids = fields.One2many("generic.tag", "category_id", "Tags", readonly=True)
    ...
    def _check_model_id(self):
        for category in self:
            ...
            if tag_model and (len(tag_model) != 1 or tag_model != category.model_id):
                raise ValidationError(
                    _("Model must be same as one used in related tags")
                )
