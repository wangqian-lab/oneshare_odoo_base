...
class GenericTagMixin(models.AbstractModel):
    ...
    def _check_tags_xor(self):
        for record in self:
            categ_counter = collections.defaultdict(self.env["generic.tag"].browse)
    ...
    def add_tag(self, code=None, name=None, create=False):
        ...
        """
        tags = self.env["generic.tag"].get_tags(self._name, code=code, name=name)

        if not tags and create:
            model = self.env["generic.tag.model"].search([("model", "=", self._name)])[
                0
            ]
