...
api_change_user_info = {
    ...
    "properties": {
        ...
        "student": {
            "type": "string"  # 如果是家长，学生名称
        },
        "avatar": {
            "type": "string"  # 头像, base64 数据
        }
    },
    "required": [
        "login",
...
class ResUserController(http.Controller):
    ...
    @http.api_route('/api/v1/user/change_password', type='apijson', methods=['PUT'], auth="user", cors='*', csrf=False,
    ...
    @http.api_route('/api/v1/user/info', type='apijson', methods=['PUT'], auth='user', cors='*', csrf=False,
                    schema=api_change_user_info)
    def change_user_info(self, **kwargs):
        ...
        if avatar:
            val.update({
                'image_1920': image_process(avatar, verify_resolution=True)
            })
        ...
        sanitize_res = \
            phone_validation.phone_sanitize_numbers_w_record([sms_number], record=None, country=default_country_id)[
                sms_number]
        ...
        if mobile_phone:
            ...
            return oneshare_json_fail_response(
                msg=f"change user: {login} mobile: {sms_number} Error: {sanitize_res['code']}", **extra)
        if ret:
            cr.commit()
