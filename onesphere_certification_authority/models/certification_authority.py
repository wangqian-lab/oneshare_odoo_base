...
class OneshareCryptographyKey(models.Model):
    ...
    revision = fields.Char(string='Rev', required=True, copy=False,
    ...
    certification_ids = fields.One2many('oneshare.certification', 'crypt_key_id', string='Certifications')
    certification_count = fields.Integer('Certification Count', compute=_compute_certification_count)
    ...
    def action_open_related_certification_tree_view(self):
        ...
        return {
            ...
            'views': [[self.env.ref('onesphere_certification_authority.oneshare_issued_cert_view_tree').id, 'tree']],
    ...
    def create(self, vals):
        if not vals.get('revision', None):
            vals.update({
                'revision': self.env['ir.sequence'].next_by_code('cryptography_key_revision')
            })
    ...
    class OneshareIssuedCertification(models.Model):
        ...
        revision = fields.Char(string='Rev', related='crypt_key_id.revision', store=True)
        secret_key = fields.Text('Secret Key', related='crypt_key_id.secret_key', store=True)
