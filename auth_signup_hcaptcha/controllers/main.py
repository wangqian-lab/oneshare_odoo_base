...
class AuthSignup(AuthSignupHome):
    ...
    def _verify_hcaptcha_token(self, ip_addr, token, action=False):
        private_key = request.env['ir.config_parameter'].sudo().get_param('hcaptcha_private_key')
        public_key = request.env['ir.config_parameter'].sudo().get_param('hcaptcha_public_key')
        if not private_key:
            return 'no_secret'
        try:
