...
redis_hosts: str = tools.config.get('redis_host', None) or os.getenv('ENV_SESSION_REDIS_HOST', 'localhost')
startup_nodes = redis_hosts.split(',')

redis_ports = int(tools.config.get('redis_port') or os.getenv('ENV_SESSION_REDIS_PORT', '6379'))
redis_db = int(tools.config.get('redis_dbindex') or os.getenv('ENV_SESSION_REDIS_DB', '1'))
redis_password = tools.config.get('redis_pass') or os.getenv('ENV_SESSION_REDIS_PASSWORD', None)
