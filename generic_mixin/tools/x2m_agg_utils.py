...
def read_counts_for(
    records, related_model, search_field, value_field, domain=None, sudo=False
):
    ...
    if records.ids:
        check_values = (
            records.ids if value_field == "id" else records.mapped(value_field)
        )
        ...
        data = RelatedModel.read_group(search_domain, [search_field], [search_field])
        mapped_data = {}
        for m in data:
            key = m[search_field]
