...
odoo.define("generic_mixin.WebClient", function (require) {
  ...
  require("web.WebClient").include({
    ...
    show_application: function () {
      ...
      this.call("bus_service", "addChannel", "generic_mixin_refresh_view");
    ...
    _generic_mixin_refresh_view__do_refresh_check: function (ctl) {
      ...
      var refresh_ids = self._generic_refresh_mixin__pending[act.res_model];
      if (!refresh_ids) {
        return false;
      }

      var actions = self._generic_refresh_mixin__pending_action[act.res_model];
      if (!actions) {
        return false;
      }
      ...
      if (
        ctl.widget.generic_mixin__is_multi_record &&
      ...
      if (
        ...
        active_ids = _.union(active_ids, [ctl.widget.renderer.state.res_id]);
      } else if (
        ctl.widget.generic_mixin__is_multi_record &&
        ctl.widget.renderer.state.res_ids
      ) {
        active_ids = _.union(active_ids, ctl.widget.renderer.state.res_ids);
      } else if (ctl.widget.initialState) {
        if (ctl.widget.initialState.res_id) {
          active_ids = _.union(active_ids, [ctl.widget.initialState.res_id]);
        } else {
          active_ids = _.union(active_ids, ctl.widget.initialState.res_ids);
        }
      }
    ...
    _generic_mixin_refresh_view__do_refresh_ctl: function (ctl) {
      if (ctl && ctl.widget) {
        ...
        if (ctl.widget.renderer.generic_refresh_view__is_compatible) {
          var refresh_ids =
            this._generic_refresh_mixin__refresh_ids[ctl.widget.modelName];
      ...
      _.each(message, function (action_data, res_model) {
        _.each(action_data, function (res_ids, action) {
          ...
          if (res_model in self._generic_refresh_mixin__pending_action) {
            self._generic_refresh_mixin__pending_action[res_model] = _.union(
              self._generic_refresh_mixin__pending_action[res_model],
              [action]
            );
          } else {
            self._generic_refresh_mixin__pending_action[res_model] = [action];
          }
