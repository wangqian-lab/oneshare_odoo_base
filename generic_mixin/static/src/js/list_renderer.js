odoo.define("generic_mixin.ListRenderer", function (require) {
  ...
  require("web.ListRenderer").include({
    ...
    _renderRow: function (record) {
      ...
      this._generic_mixin_refresh_view__visualize_changes($tr, record.res_id);
      return $tr;
    },

    _generic_mixin_refresh_view__visualize_changes: function ($tr, res_id) {
      if (
        this._generic_refresh_mixin__refresh_ids.create &&
        this._generic_refresh_mixin__refresh_ids.write
      ) {
        if (this._generic_refresh_mixin__refresh_ids.create.includes(res_id)) {
          setTimeout(function () {
          ...
          this._generic_refresh_mixin__refresh_ids.write.includes(res_id)
        ) {
          setTimeout(function () {
