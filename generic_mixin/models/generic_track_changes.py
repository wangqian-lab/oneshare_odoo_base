...
class FieldChangeHandler:
    ...
    def __init__(self, obj, method_name):
        ...
        self.post_write_fields = self._get_tracking_fields("_post_write_fields")
        self.pre_create_fields = self._get_tracking_fields("_pre_create_fields")
        self.post_create_fields = self._get_tracking_fields("_post_create_fields")
    ...
    def _get_tracking_fields(self, fields_attr):
        return get_method_fields_via_mro(self._obj, self._method_name, fields_attr)
    ...
    def validate(self):
        ...
        if self.pre_write_fields and self.post_write_fields:
            _logger.warning(
                ...
                "@post_write at same time! Method: %s",
                self._method_name,
            )
        ...
        if self.is_pre_create and self.is_post_create:
            _logger.warning(
                ...
                "@post_create at same time! Method: %s",
                self._method_name,
            )
        ...
        for name in self.pre_write_fields:
            if name not in self._obj._fields:
                _logger.warning(
                    "@pre_write%r (%s) parameters must be " "field name (%s)",
                    tuple(self.pre_write_fields),
                    self._method_name,
                    name,
                )
        ...
        for name in self.post_write_fields:
            if name not in self._obj._fields:
                _logger.warning(
                    "@post_write%r (%s) parameters must be " "field name (%s)",
                    tuple(self.post_write_fields),
                    self._method_name,
                    name,
                )
        ...
        for name in self.pre_create_fields:
            if name not in self._obj._fields:
                _logger.warning(
                    "@pre_create%r (%s) parameters must be " "field name (%s)",
                    tuple(self.pre_write_fields),
                    self._method_name,
                    name,
                )
        ...
        for name in self.post_create_fields:
            if name not in self._obj._fields:
                _logger.warning(
                    "@post_create%r (%s) parameters must be " "field name (%s)",
                    tuple(self.post_write_fields),
                    self._method_name,
                    name,
                )
...
class GenericMixInTrackChanges(models.AbstractModel):
    ...
    def _init_constraints_onchanges(cls):
        ...
        return super(GenericMixInTrackChanges, cls)._init_constraints_onchanges()
            ...
            for record in self:
                for field in changed_fields:
                    ...
                    new_value = self._fields[field].convert_to_record(
                        self._fields[field].convert_to_cache(vals[field], self), self
                    )
                    if old_value != new_value:
                        changes[record.id][field] = FieldChange(old_value, new_value)
        return dict(changes)
    ...
    def write(self, vals):
        ...
        for record in self:
            if record.id in updates:
                super(GenericMixInTrackChanges, record).write(updates[record.id])
            if record.id in changes:
    ...
    def _create__get_changed_fields(self, vals):
        ...
        for fname, fval in vals.items():
            ...
            if (
                ...
                and isinstance(fval, str)
                and len(fval) <= DATETIME_LENGTH
            ):
                try:
            ...
            new_value = self._fields[fname].convert_to_record(
                self._fields[fname].convert_to_cache(fval, self), self
            )
            if old_value != new_value:
