...
class GenericMixinDataUpdatable(models.AbstractModel):
    ...
    @api.depends(
        "ir_model_data_ids",
        "ir_model_data_ids.noupdate",
        "ir_model_data_ids.name",
        "ir_model_data_ids.module",
    )
    def _compute_ir_model_data(self):
        ...
        ir_model_data_map = {m.res_id: m for m in ir_model_data}
        for record in self:
            data_rec = ir_model_data_map.get(
                record.id, self.sudo().env["ir.model.data"].browse()
            )
