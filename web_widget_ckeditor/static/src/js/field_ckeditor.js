...
odoo.define("web_widget_ckeditor.field_ckeditor", function (require) {
  ...
  const CKEditorLanguageCode =
    session.user_context.lang === "zh_CN"
      ? "zh-cn"
      : session.user_context.lang.split("_")[0];
  const loadCKEditorLanguagePromise =
    loadCKEditorLanguageSource(CKEditorLanguageCode);
  ...
  const FieldHtmlCKEditor = basic_fields.DebouncedField.extend(
    TranslatableFieldMixin,
    {
      ...
      isSet: function () {
        ...
        const value =
          this.value && this.value.split("&nbsp;").join("").replace(/\s/g, "");
        return (
          value &&
          value !== "<p></p>" &&
          value !== "<p><br></p>" &&
      ...
      _getCKEditorToolbarItems: async function () {
        try {
          ...
          console.warn("Unable to use CKEditor toolbar configuration: ", error);
          console.warn(
      ...
      _getCKEditorConfig: async function () {
        const o = await this._getCKEditorToolbarItems();
        const items = o.concat(["codeBlock", "findAndReplace"]);
        const res = {
          toolbar: {
            items: items,
      ...
      _textToHtml: function (text) {
        ...
        try {
          ...
          if (value.match(/^\s*$/)) {
            ...
            value =
              "<p>" + value.split(/<br\/?>/).join("<br/></p><p>") + "</p>";
            value = value
              .replace(/<p><\/p>/g, "")
              .replace("<p><p>", "<p>")
              .replace("<p><p ", "<p ")
